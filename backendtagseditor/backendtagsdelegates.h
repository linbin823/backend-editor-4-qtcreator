...
class TagRWStrategyDelegate: public QItemDelegate{
    Q_OBJECT
public:
    TagRWStrategyDelegate(RTDataTableModel* model, QObject* parent =0): QItemDelegate(parent), _pModel(model){}
...
class TagTypeDelegate: public QItemDelegate{
    Q_OBJECT
public:
    TagTypeDelegate(RTDataTableModel* model, QObject* parent =0): QItemDelegate(parent), _pModel(model){}
    QWidget* createEditor(QWidget *parent, const QStyleOptionViewItem &option,
                          const QModelIndex &index) const override{
        Q_UNUSED(option)
        QComboBox* editor = new QComboBox(parent);

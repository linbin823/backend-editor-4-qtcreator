...
<<<<<<< HEAD
=======
void ModbusMasterProtocolDriver::_initDataUnites() {
    _tagAddresses.clear();
    _dataUnits.clear();
}
...
DriverInfo ModbusMasterProtocolDriver::driverInfo()const{
    ...
    info.microVersion = QString::number(2);
    return info;
}
...
int ModbusMasterProtocolDriver::start(){
    ...
    if(type == Serial){
        ...
        _pModbusDevice = modbus_new_tcp(_parameters[NetworkAddressParameter].toString().toUtf8().data(),
...
//    modbus_set_error_recovery(_pModbusDevice, modbus_error_recovery_mode(MODBUS_ERROR_RECOVERY_LINK | MODBUS_ERROR_RECOVERY_PROTOCOL));
    ...
    if (modbus_connect(_pModbusDevice) == -1) {
        emit msgEventString(tr("启动失败。无法连接本地设备！%1。请检查权限").arg(modbus_strerror(errno)));
        _setState(STAT_ERR);
...
QList<ModbusDataUnit> ModbusMasterProtocolDriver::_prepareDataUnit(const QList<TagAddress *> &list, int dataType) const {
...
void ModbusMasterProtocolDriver::_unitProcesss(ModbusDataUnit unit, int dataType, int RWStrategy) {
    if (!_pModbusDevice) return;
    if (_state != STAT_RUN) return;
...
void ModbusMasterProtocolDriver::_readValues(const QList<TagAddress *> &list, const ModbusDataUnit& unit, const int type) {
...
void ModbusMasterProtocolDriver::_readValue(TagAddress *t, const ModbusDataUnit& unit, uint index, const int type) {
...
void ModbusMasterProtocolDriver::_writeValues(const QList<TagAddress *>& list, ModbusDataUnit& unit, const int type) {
    int startAddress = unit.startAddress();
    int unitDataCount;
    switch (type) {
    case Coils:
...
void ModbusMasterProtocolDriver::_writeValue(TagAddress *t, ModbusDataUnit& unit, uint index, const int type) {
...
void ModbusMasterProtocolDriver::_errorProcess(const QString& msg) {
    emit msgEventString(msg);
    qDebug()<<msg<<errno;

    //need restart
    if (errno == EMBXSFAIL || errno == EIO || errno == ECONNREFUSED) {
    ...
    //need flush
    else if (errno == EMBBADCRC || errno == EMBXMEMPAR || errno == EMBMDATA || errno == ECONNRESET) {
